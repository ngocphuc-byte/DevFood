...
import {
  ...
  getTotal,
  getIdVoucher,
  onHandlerUpdateQuantityDown,
  onHandlerUpdateQuantityUp,
  getVoucher,
  RemoveItemCart,
  updateTotal,
  checkVoucher,
  ...
  return (
    <View style={{width: '100%', height: '100%', flexDirection: 'row'}}>
      ...
      <View style={styles.containerPrice}>
        <Text style={[styles.textFood, {width: '100%', textAlign: 'right'}]}>
          {item.price}.000
        </Text>
      </View>
    </View>
  ...
  return (
    <View style={styles.containerDelete}>
      <TouchableOpacity style={styles.buttonDelete} onPress={onHandlerRemove}>
        <Text style={[styles.textInfo, {color: white, fontSize: 18}]}>
          Delete
        </Text>
      </TouchableOpacity>
    </View>
      ...
      .then(res => {
        if (res.length > 0) {
          if (voucher.minprice <= total) {
            ...
            Alert.alert(
              'WARNING',
              'Bạn không đủ điều kiện sử dụng voucher của chúng tôi !',
            );
          }
        } else {
    ...
    <View style={styles.containerVoucherModal}>
      <View style={styles.containerInfoVoucher}>
        ...
        <Text style={styles.textInfo}>
          Giảm tối đa {voucher.discount}k đơn hàng tối thiểu {voucher.minprice}k
        </Text>
      </View>
  ...
  return (
    <View style={styles.containerVoucherModal}>
      ...
      <NameVoucher
        ...
        total={total}
        onHandlerRefresh={onHandlerRefresh}
        setIdDetailVoucher={setIdDetailVoucher}
      />
    </View>
  );
};
    ...
    <View style={styles.containerBody}>
      <View style={styles.containerCart}>
        <FlatList
          ...
          renderItem={({item, index}) =>
            renderFood(item, index, setTotal, total, Account.idAccount)
          }
      ...
      <View style={styles.containerVoucher}>
        <TouchableOpacity
          ...
          {apply == 0 ? (
            <Text style={[styles.textCount, {color: '#f12'}]}>
              Bạn muốn sử dụng voucher chứ ?
            </Text>
          ) : (
            <Text style={[styles.textCount, {color: '#f12'}]}>
              Bạn đang sử dụng voucher giảm {apply}k
            </Text>
          )}
        </TouchableOpacity>
      </View>
      <View style={styles.containerTotal}>
        <Text
          style={[
            styles.textCount,
            {color: greenlight, fontSize: 30, height: '100%'},
          ]}>
          Total
        </Text>
        {Number(total - apply).toString().length >= 4 ? (
          <Text
            ...
            {total.toString().slice(0, 1)}.{total.toString().slice(1, 4)}.000
          </Text>
        ) : (
          <Text
            style={[
